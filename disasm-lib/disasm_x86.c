...
/*
X86_GetInstruction函数的主要功能是从给定的地址开始，解析并获取一条x86或x64指令。
它会填充INSTRUCTION结构体中的相关字段，包括操作码、操作数、指令长度等信息。

以下是X86_GetInstruction函数的具体逻辑：

初始化指令结构体INSTRUCTION。
解析指令前缀（如操作数大小前缀、地址大小前缀等）。
解析操作码，并根据操作码确定指令类型。
解析ModR/M字节和SIB字节（如果存在），以确定操作数的类型和地址模式。
解析操作数，并填充INSTRUCTION结构体中的相关字段。
处理特殊指令（如分支指令、调用指令等），并计算目标地址。
返回解析后的指令结构体。
*/
//////////////////////////////////////////////////////////
// Instruction decoder
